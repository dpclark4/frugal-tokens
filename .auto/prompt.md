...
- Kept split experiment: `/api/work-rhythm` now returns summary/diagnostics and `/api/work-rhythm/days` supplies the calendar/timelines after the summary resolves. The critical median was 32.81 ms with 129.6 KB (prior all-data baseline 35.39 ms and 265.4 KB), summary aggregation fell to 1.9 ms, and complete data was 46.04 ms because the deferred days request adds about 13 ms. Work-rhythm response equality is intentionally exempt.
- Kept schedule experiment: below-the-fold Session shape, usage tooltip data, and Cache misses now wait for the activity and work-rhythm summary requests to settle. The phase-model baseline is 16.92 ms critical / 46.20 ms complete with all response guards passing; it intentionally trades later secondary completion for a much earlier visible core.
- Kept payload experiment: work-rhythm session diagnostics now travels with the deferred days response instead of the initial summary. Critical bytes fell from 107.1 KB to 37.9 KB and the summary response from 73.0 KB to 3.8 KB; critical_ms moved 16.92 → 15.66 ms with complete bytes unchanged and all guarded responses passing.
- Kept materialization experiment: activity, work-rhythm, and deferred days now share a 25 ms full overview-rollup cache, with the core callers requesting the same superset and the cache cleared after sync. Critical_ms reached 11.50 ms and complete_ms 35.52 ms; all guarded responses passed.
- Validated the additive root-turn covering index against the actual benchmark schema (the first measurement ran before that migration was applied). With the index active, critical_ms was 11.22 ms and complete_ms 34.42 ms; all guarded responses passed.
- Kept the scheduling refinement: calendar/details now waits for both core responses, not only the work-rhythm summary, preventing deferred SQLite work from delaying activity. The new phase baseline is 11.07 ms critical / 37.70 ms complete with all guards passing.
