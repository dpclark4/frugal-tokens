...
export function aggregateOverviewRollups(
  ...
  for (const root of roots) {
    const rootKey = String(root.rootSessionID);
    const executionIntervals = root.overview.executionIntervals.map(
      (interval) => ({
        start: interval.startedAt,
        end: interval.executionEndAt,
      }),
    );
    ...
    for (const day of days) {
      ...
      if (plainDate.dayOfWeek <= 5) activeWeekdays.add(day.date);
      else weekendDates.add(day.date);
      const workedOn = dailySessions.get(day.date) ?? new Set<string>();
      workedOn.add(rootKey);
      dailySessions.set(day.date, workedOn);
      dailyTurns.set(day.date, (dailyTurns.get(day.date) ?? 0) + day.turns);
      dailySpend.set(day.date, (dailySpend.get(day.date) ?? 0) + day.cost);

      sessionTurns += day.turns;
      sessionInput += day.input;
      sessionCacheRead += day.cacheRead;
      sessionSpend += day.cost;
      peakContext = Math.max(peakContext, day.peakContext);
      firstTurn = Math.min(firstTurn, day.firstTurnAt);
      if (day.lastCallAt !== undefined) {
        lastCall = Math.max(lastCall, day.lastCallAt);
      }
      ...
      for (const model of day.models) {
        ...
        bucket.spend += model.cost;
        bucket.hasUnpricedCost ||= model.hasUnpricedCost;
        models.set(model.model, bucket);
      }
    }

    profileTurns.push(sessionTurns);
